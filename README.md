...
- “重要提醒设置”把跟随事项时间、目标日提醒和周期提醒放在同一处；周期提醒支持每天、每周、每两周、每月、每年，或自定义每隔 N 天、周、月、年提醒一次。复杂计划启用后，简单提醒自动不可叠加。
- 三种重要提醒方式分别保存设置，但只能有一种被选中生效；分类按钮只负责查看，右侧固定按钮负责选中当前分类；未选中的方式仍可查看和调整，界面会以置灰状态显示，保存后草稿也会保留。
