...
class WorkspaceEditor(QWidget):
    ...
    def _refresh_status(self, *_unused) -> None:
        ...
        saved_copy = self._task_id is not None and not self.is_dirty()
        can_copy = saved_copy and not self._operation_feedback_timer.isActive()
        self.duplicate_button.setEnabled(can_copy)
        if not saved_copy:
            tooltip = "请先保存当前修改后，再选择继续处理方式。"
        elif not can_copy:
            tooltip = "操作已完成，请稍候再试。"
        else:
            tooltip = "保留当前记录，新建一条后续事项；分项步骤重新开始。"
    ...
    def show_operation_feedback(self, text: str, *, action: str = "duplicate") -> None:
        ...
        self._hide_operation_feedback()
        target.setText(text)
        target.setVisible(True)
        self._operation_feedback_timer.start(3500)
        self._refresh_status()

    def _hide_operation_feedback(self) -> None:
    ...
    def clear(self) -> None:
        ...
        self.custom_offset_combo.setCurrentIndex(0)
        self.important_schedule.set_task_date(self.date_edit.date())
        self.important_schedule.clear_selection()
        self._schedule_touched = False
